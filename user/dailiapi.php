...
if($username == ""){
    ...
    if($gmbb==''){
        ...
        $cmd2 = 'importance=&config_id=&uid='.$uid.'&title='.urlencode('变态圣遗物到账啦！').'&content='.urlencode('尊敬的用户:'.$username.'\n感谢你在GM平台购买变态圣遗物成功！\nCDK兑换地址:http://gh02.miquanbao.com').'&sender='.urlencode('原神GM平台').'&expire_time='.$yjdqsj.'&is_collectible=False&item_limit_type=2&tag=&source_type=&item_list='.$data2.':1&cmd=1005&ticket='.$yjfssj;
    }elseif($gmbb=='2'){
        $gmbb="摩卡2服";
        $gmdz1="http://103.45.185.220:20011/api?uid=";
        $yjdz="http://103.45.185.220:20011/api?";
        $cmd2 = 'importance=&config_id=&uid='.$uid.'&title='.urlencode('变态圣遗物到账啦！').'&content='.urlencode('尊敬的用户:'.$username.'\n感谢你在GM平台购买变态圣遗物成功！\nQQ交流群:513364857\n欢迎各位大佬加群交流').'&sender='.urlencode('摩卡原神GM平台').'&expire_time='.$yjdqsj.'&is_collectible=False&item_limit_type=2&tag=&source_type=&item_list='.$data2.':1&cmd=1005&ticket='.$yjfssj;
    }elseif($gmbb=='3'){
        $gmbb="摩卡2服";
        $gmdz1="http://103.45.185.220:20011/api?uid=";
        $yjdz="http://103.45.185.220:20011/api?";
        ...
        $cmd2 = 'importance=&config_id=&uid='.$uid.'&title='.urlencode('变态圣遗物到账啦！').'&content='.urlencode('尊敬的用户:'.$username.'\n感谢你在GM平台购买变态圣遗物成功！').'&sender='.urlencode('摩卡原神GM平台').'&expire_time='.$yjdqsj.'&is_collectible=False&item_limit_type=2&tag=&source_type=&item_list='.$data2.':1&cmd=1005&ticket='.$yjfssj;
    }
    $zxurl = $yjdz.$cmd2;
    ...
    if($userzt == '1'){
        ...
        if ($json['msg']=='mail has been sent, do not re_send mail') {
            header("Location: gf11.php?result=8&ycuid=".$uid); 
            header("target: _self");
            exit ;
            ...
            $result = mysqli_query($root,$sql);
            header("Location: gf11.php?result=1&ycuid=".$uid); 
            header("target: _self");
        }else{
            header("Location: gf11.php?result=8&ycuid=".$uid); 
            header("target: _self");
        ...
        if ($json['msg']=='mail has been sent, do not re_send mail') {
            ...
            header("Location: gf11.php?result=9&ycuid=".$uid); 
            header("target: _self");
            exit ;
        }
    }
