    ...
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>高级权限CDK生成 <small>高级权限CDK生成</small></h5>
            ...
            if($_POST['number']==''){
                ...
                //echo"<script>swal('生成成功','','success');</script>";
                $cdk=$guid;
                $wp=$_POST['giveid'];
                $sl=$_POST['number'];
                $bt="权限开通成功通知";
                $nr="感谢你的赞助，目前以为你开通高级权限，请使用/mojo命令获取权限邮件！";
                $sc=$_SESSION['name'];
                include "cus_check.php";
                $sql = "select * from cdk_qx where cdk LIKE '$cdk'";
                $result = mysqli_query($root,$sql);             //查询SELECT * FROM `cdk_wp` WHERE `cdk` LIKE 'zpl'
                $no = mysqli_num_rows($result);   
                $sj = time(); 
                if($no)
                {
                    echo"<script>swal('生成失败','CDK重复，请重新生成新的CDK','warning');</script>";
                    exit;
                }
                $sql = "insert into cdk_qx (cdk,wp,sl,bt,nr,uid,sc,sj,zt) VALUE ('$cdk','$wp','$sl','$bt','$nr','','$sc','$sj','0')";
                //INSERT INTO `cdk_wp` (`id`, `cdk`, `wp`, `sl`, `bt`, `nr`, `uid`, `sc`, `zt`, `sj`) VALUES (NULL, '8251633168', '201', '1000', '112', '3341', NULL, 'txy666', '0', '');
                $result = mysqli_query($root,$sql);
                echo"<script>swal('生成成功，请手动复制以下卡密','$guid','success');</script>";
            }
?>
                    </div>
                </div>
